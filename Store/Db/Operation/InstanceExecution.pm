...
class Workflow::Store::Db::Operation::InstanceExecution {
    ...
    id_by => [
        execution_id => { is => 'NUMBER', len => 11, column_name => 'WORKFLOW_EXECUTION_ID' },
    ],
    has => [
        end_time      => { is => 'TIMESTAMP', len => 20, is_optional => 1 },
        exit_code     => { is => 'NUMBER', len => 5, is_optional => 1 },
        instance_id   => { is => 'NUMBER', len => 11, column_name => 'WORKFLOW_INSTANCE_ID' },
        ...
        dispatch_identifier => { is => 'VARCHAR2', len => 10, column_name => 'DISPATCH_ID', is_optional => 1 },
    ],
