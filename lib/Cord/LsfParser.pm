...
Cord::LsfParser - Parse LSF rusage into a Cord::Resource object

=head1 SYNOPSIS

Should be called statically e.g.:

my $resource = Cord::LsfParser::get_resource_from_lsf_resource("-R 'select[tmp>1040]...");
...
sub get_resource_from_lsf_resource {
    ...
    if (eval { $lsf_resource->isa("Cord::LsfParser"); }) {
        # called as method. shift again to get our lsf rusage
        $lsf_resource = shift;
    }
    my $resource = Cord::Resource->create();
