...
Cord::Time - a few useful time-related functions.

=head1 SYNOPSIS

  ##- use Cord::Time;

  Cord::Time->config(time => '%H:$M:%S');
  $format = Cord::Time->config('time');

  $date = Cord::Time->today;

  $date_time = Cord::Time->now;

  $ds = Cord::Time->timediff($t1, $t2);

  $rv = Cord::Time->compare_dates($d1, $d2);

  ($sec, $min, $hour, $day, $mon, $year)
      = Cord::Time->datetime_to_numbers($datetime);

  $datetime = Cord::Time->numbers_to_datetime($sec, $min, $hour,
                                             $day, $mon, $year);
...
=item config
  ...
  Cord::Time->config(datetime => '%Y-%m-%d %H:%M:%S');
  $dt_format = Cord::Time->config('datetime');
  %config = Cord::Time->config;
...
=item from_integer
  ...
  $date = Cord::Time->from_integer($time);
...
sub compare_dates
{
    ...
    my ($s1, $n1, $h1, $d1, $m1, $y1) = Cord::Time->datetime_to_numbers($date1);
    my ($s2, $n2, $h2, $d2, $m2, $y2) = Cord::Time->datetime_to_numbers($date2);
...
=item datetime_to_time
  ...
  $time = Cord::Time->datetime_to_time($datetime);
...
=item add_date_delta_days
    ...
    $date = Cord::Time->add_date_delta_days($date, $days);
