...
sub create {
    ...
    $_create_model_input_tmpl ||= UR::BoolExpr::Template->resolve(
                                        'Cord::OperationType::ModelInput',
                                        #'id','input_properties','output_properties'
                                        'id'
                                    )->get_normalized_template_equivalent();

    my $input_op_type_id = Cord::OperationType::ModelInput->__meta__->autogenerate_new_object_id();
    my $input_op_type = UR::Context->create_entity('Cord::OperationType::ModelInput',
    ...
    my $input_op = UR::Context->create_entity('Cord::Operation',
                                $_add_operation_tmpl->get_rule_for_values(
                                    Cord::Operation->__meta__->autogenerate_new_object_id(),
                                    'input connector',
                                    $self->id,
                                    $input_op_type_id));
    $input_op->_post_create();

    my $output_op_type_id = Cord::OperationType::ModelOutput->__meta__->autogenerate_new_object_id();
    my $output_op_type = UR::Context->create_entity('Cord::OperationType::ModelOutput',
                                                    $_create_model_input_tmpl->get_rule_for_values(
                                                            $output_op_type_id,
    ...
    my $output_op = UR::Context->create_entity('Cord::Operation',
                                $_add_operation_tmpl->get_rule_for_values(
                                    Cord::Operation->__meta__->autogenerate_new_object_id(),
                                    'output connector',
                                    $self->id,
...
sub validate {
    ...
    my $self_id = $self->id;
    my $get_ops_for_name_tmpl = UR::BoolExpr::Template->resolve('Cord::Operation', 'name', 'workflow_model_id')->get_normalized_template_equivalent();
    foreach my $operation (@operations) {  ## unique names
        my $rule = $get_ops_for_name_tmpl->get_rule_for_values($operation->name, $self_id);
        my @ops_with_this_name = Cord::Operation->get( $rule );
    ...
    my $resolve_properties_for_optype = sub {
            ...
            my $optype = Cord::OperationType->get($optype_id);
...
$DB::single=1;
    foreach my $link ($self->links) {
        ...
        my $left_optype_id = Cord::Operation->get($link->{'left_workflow_operation_id'})->{'workflow_operationtype_id'};
        ...
        my $right_optype_id = Cord::Operation->get($link->{'right_workflow_operation_id'})->{'workflow_operationtype_id'};
...
$DB::single=1;
    ...
    for my $link ( Cord::Link->get(workflow_model_id => $self->id)) {
    ...
    my @operations_to_check = Cord::Operation->get(workflow_model_id => $self->id,  # operations in this workflow
    ...
    while(@operations_to_check) {               # while S is non-empty do
        ...
        my @outgoing_links = Cord::Link->get(left_workflow_operation_id => $op_id);
