...
class Cord::Operation {
    has => [
        ...
        notify_url => { is => 'String', is_optional => 1, doc => 'URL that will be sent a GET request when any sub-status changes in this Cord or Operation, multiple urls may be seperated by spaces' }
    ]
};
...
sub dependent_operations {
    ...
    $dependent_operations_tmpl
        ||= UR::BoolExpr::Template->resolve('Cord::Link', 'left_workflow_operation_id')->get_normalized_template_equivalent();

    my %operations = map { $_->right_operation->id => $_->right_operation }
                     Cord::Link->get($dependent_operations_tmpl->get_rule_for_values($self->id));
...
sub depended_on_by {
    ...
    $depended_on_by_tmpl
        ||= UR::BoolExpr::Template->resolve('Cord::Link', 'right_workflow_operation_id')->get_normalized_template_equivalent();

    my %operations = map { $_->left_operation->id => $_->left_operation }
                     Cord::Link->get($depended_on_by_tmpl->get_rule_for_values($self->id));
