            ...
            start_jobs => sub {
                ...
                while ($heap->{job_count} < $heap->{job_limit}) {
                    ...
                    if ($payload->{shortcut_flag}) {
                        ...
                        # $resource is a Cord::Resource object
                        ...
                        my $command = sprintf("annotate-log $^X -e '%s use %s; use %s; use Cord::Server::Worker; Cord::Server::Worker->start(\"%s\", %s)'", $libstring, $namespace, $payload->{operation_type}->command_class_name, hostname, $port_number);
                        ...
                        my $dispatcher = Cord::Dispatcher->get();
