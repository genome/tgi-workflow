    ...
    foreach my $input_name (@all_inputs) {
          ...
          )
        {
          ...
          VALCHECK: foreach my $v (@$vallist) {
                if (eval { $v->isa('Cord::Link::Instance') }) {
                    if ( $v->broken ) {
...
sub treeview_debug {
    ...
    while ( my ( $k, $v ) = each( %{ $self->input } ) ) {
        ...
        foreach my $dv (@$vals) {
            my $vc = $dv;
            if (eval { $dv->isa('Cord::Link::Instance' ) }) {
                $vc =
        ...
        if ( defined $dep->input ) {
            while ( my ( $k, $v ) = each( %{ $dep->input } ) ) {
                if (eval { $v->isa('Cord::Link::Instance') }) {
                    ...
                    foreach my $vv (@$v) {
                        if (eval { $vv->isa('Cord::Link::Instance') })
                        {
...
#    if ($self->name eq 'Example Inner Cord') {
...
sub depended_on_by {
    ...
    foreach my $v ( values %{ $self->input } ) {
        if (eval { $v->isa('Cord::Link::Instance') } && !$v->broken ) {
...
sub create_peers {
      ...
      )
    {
        ...
        if (eval { $peer->isa('Cord::Model::Instance') }) {
        ...
        foreach my $dep (@deps) {
            my @k = grep {
                if (
                    ...
                    && 
                    eval { 
                        $dep->input->{$_}->[0]->isa('Cord::Link::Instance')                     }
