...
package Workflow::Time;

=pod

=head1 NAME

Workflow::Time - a few useful time-related functions.

=head1 SYNOPSIS

  ##- use Workflow::Time;

  Workflow::Time->config(time => '%H:$M:%S');
  $format = Workflow::Time->config('time');

  $date = Workflow::Time->today;

  $date_time = Workflow::Time->now;

  $ds = Workflow::Time->timediff($t1, $t2);

  $rv = Workflow::Time->compare_dates($d1, $d2);

  ($sec, $min, $hour, $day, $mon, $year)
      = Workflow::Time->datetime_to_numbers($datetime);

  $datetime = Workflow::Time->numbers_to_datetime($sec, $min, $hour,
                                             $day, $mon, $year);
...
=item config
  ...
  Workflow::Time->config(datetime => '%Y-%m-%d %H:%M:%S');
  $dt_format = Workflow::Time->config('datetime');
  %config = Workflow::Time->config;
...
=item from_integer
  ...
  $date = Workflow::Time->from_integer($time);
...
=item datetime_to_time
  ...
  $time = Workflow::Time->datetime_to_time($datetime);
...
=item add_date_delta_days
    ...
    $date = Workflow::Time->add_date_delta_days($date, $days);
